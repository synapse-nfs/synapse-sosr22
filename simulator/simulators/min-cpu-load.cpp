...
#define MAX_PORT_DATAPLANE 40000
...
int main(int argc, char *argv[]) {
  ...
  std::unordered_map<crc, pkt_t> ite_reg;
  ...
  for (auto &pkt : pkt_data.pkts) {
    ...
    if (ite_key_crc_found != ite_reg.end()) {
      ...
      if (stored_flow.flow != pkt.flow) {
        ...
        stored_flow.ts = pkt.ts;
      }

      continue;
    }

    if (port_allocator_reg >= MAX_PORT_DATAPLANE) {
      if (flows.size() >= MAX_PORTS) {
        fprintf(stderr, "\nError: no more available ports.\n");
        exit(1);
      }
  ...
  printf("Expirations reg     %lu (%lu fpm) \n", expirations_reg,
         (uint64_t)(60 *
                    (((double)expirations_reg) / ((double)elapsed / BILLION))));
  printf("Flows in registers  %lu (%.5f%%)\n", flows_in_registers,
         100 * (((double)flows_in_registers) /
                (sent_to_controller + flows_in_registers)));
  printf("Flows in controller %lu (%.5f%%)\n", sent_to_controller,
         100 * (((double)sent_to_controller) /
                (sent_to_controller + flows_in_registers)));
